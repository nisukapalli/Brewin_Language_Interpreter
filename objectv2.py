...
from type_valuev2 import Type, Value, Exception
...
class ObjectDef:
    ...
    def __add_locals_to_env(self, env, var_defs, line_number):
        for var_def in var_defs:
            ...
            is_template = self.interpreter.check_template(var_def[0])
    ...
    def __execute_call(self, env, code):
        status, return_value = self.__execute_call_aux(env, code, code[0].line_num)
        return status, return_value
    ...
    def __execute_try(self, env, return_type, code):
        ...
        if status == ObjectDef.STATUS_THROW:
            ...
            if not env.create_new_symbol(var_name):
               self.interpreter.error(
                   ErrorType.NAME_ERROR,
                   "exception variable name already used " + var_name,
                   code[0].line_num
               )
            var_def = VariableDef(var_type, var_name, Value(var_type, return_value.value()))
            env.set(var_name, var_def)
    ...
    def __execute_throw(self, env, return_type, code):
        exception = self.__evaluate_expression(env, code[1], code[0].line_num)[1]
        if exception.type().type_name != InterpreterBase.STRING_DEF:
            self.interpreter.error(
                    ErrorType.TYPE_ERROR,
                    "must throw a string argument ",
                    code[0].line_num,
                )
        return ObjectDef.STATUS_THROW, Exception(exception.value())
    ...
    def __execute_print(self, env, code):
        ...
        for expr in code[1:]:
            ...
            term = self.__evaluate_expression(env, expr, code[0].line_num)[1]

            try:
                typ = term.type()
            except:
                return ObjectDef.STATUS_THROW, term
            val = term.value()
    ...
    def __evaluate_expression(self, env, expr, line_num_of_statement):
        ...
        if type(expr) is not list:
            ...
            if var_def is not None:
                return status, self.__propagate_type_to_null(var_def)
            elif expr in self.fields:
                return status, self.__propagate_type_to_null(
                    self.fields[expr]
        ...
        if operator in self.binary_op_list:
            status1, operand1 = self.__evaluate_expression(env, expr[1], line_num_of_statement)
            status2, operand2 = self.__evaluate_expression(env, expr[2], line_num_of_statement)
         
            if status1 == ObjectDef.STATUS_THROW:
                return status1, operand1
            if status2 == ObjectDef.STATUS_THROW:
            ...
            if (
                ...
                return status, self.binary_ops[InterpreterBase.INT_DEF][operator](
                    operand1, operand2
                )
            ...
            if (
                ...
                return status, self.binary_ops[InterpreterBase.STRING_DEF][operator](
                    operand1, operand2
                )
            if (
                operand1.type() == operand2.type()
                ...
                return status, self.binary_ops[InterpreterBase.BOOL_DEF][operator](
                    operand1, operand2
                )
            ...
            if self.interpreter.check_type_compatibility(
                ...
                return status, self.binary_ops[InterpreterBase.CLASS_DEF][operator](
                    operand1, operand2
                )
            self.interpreter.error(
                ErrorType.TYPE_ERROR,
        ...
        if operator in self.unary_op_list:
            status1, operand = self.__evaluate_expression(env, expr[1], line_num_of_statement)

            if status1 == ObjectDef.STATUS_THROW:
                return status1, operand
            ...
            if operand.type() == ObjectDef.BOOL_TYPE_CONST:
                ...
                return status, self.unary_ops[InterpreterBase.BOOL_DEF][operator](operand)
        ...
        if operator == InterpreterBase.NEW_DEF:
            return status, self.__execute_new_aux(env, expr, line_num_of_statement)

    # (new classname)
    def __execute_new_aux(self, env, code, line_num_of_statement):
        class_name = code[1]
        is_template = self.interpreter.check_template(code[1])
    ...
    def __execute_call_aux(self, env, code, line_num_of_statement):
        ...
        if obj_name == InterpreterBase.ME_DEF:
            ...
            obj_val = self.__evaluate_expression(env, obj_name, line_num_of_statement)[1]
            if obj_val.is_null():
                self.interpreter.error(
        ...
        actual_args = []
        for expr in code[3:]:
            status, return_value = self.__evaluate_expression(env, expr, line_num_of_statement)
            if status == ObjectDef.STATUS_THROW:
                return status, return_value
            actual_args.append(
                return_value
