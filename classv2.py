...
class ClassDef:
    ...
    def __create_variable_def_from_field(self, field_def):
        ...
        if not self.interpreter.check_type_compatibility(
            ...
            is_template = self.interpreter.check_template(field_def[1])


            if not is_template:
                found = 0
                for t in self.interpreter.template_class_index:
                    template_def = self.interpreter.template_class_index[t]
                    if field_def[1] == template_def.object_name:
                        found = 1
                        break
                if not found:
                    self.interpreter.error(
...
class TemplateClassDef:
    def __init__(self, class_source, interpreter, template_types):
        self.interpreter = interpreter
        self.name = class_source[1]
        self.class_source = class_source
        self.template_types = template_types
        self.object_name = class_source[1]
    ...
    def __create_variable_def_from_field(self, field_def):
        ...
        if not self.interpreter.check_type_compatibility(
            ...
            if field_def[1] not in self.template_types and field_def[1] != self.object_name:
                found = 0
                for t in self.interpreter.template_class_index:
                    template_def = self.interpreter.template_class_index[t]
                    if field_def[1] == template_def.object_name:
            ...
            '''
            elif var_def.value != None and var_def.value.v != None:
                self.interpreter.error(
                    ErrorType.TYPE_ERROR,
                    "invalid type/type mismatch with field " + field_def[2],
                    field_def[0].line_num,
    ...
    # are not duplicated
    def __check_method_names_and_types(self, method_def):
        typ = method_def.return_type
        if not self.interpreter.is_valid_type(
            typ.type_name
        ) and typ != Type(InterpreterBase.NOTHING_DEF): #checks that return type isn't a defined type or void
            if typ.type_name not in self.template_types and typ.type_name != self.object_name:
                self.interpreter.error(
                    ErrorType.TYPE_ERROR,
                    "invalid return type for method " + method_def.method_name,
                    method_def.line_num,
        ...
        for param in method_def.formal_params:
            ...
            if not self.interpreter.is_valid_type(param.type.type_name):
                if param.type.type_name not in self.template_types and param.type.type_name != self.object_name:
                    self.interpreter.error(
                        ErrorType.TYPE_ERROR,
                        "invalid type for parameter " + param.name,
                        method_def.line_num,
